...
export function TRDetail({ mode }: { mode: 'new' | 'edit' }) {
  ...
  const { data: ownerSub } = useQuery({
    queryKey: ['scope-owner', 'tr', form.ca_id],
    enabled: !!form.ca_id,
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                value={form.ca_id ?? ''}
                onChange={async (e) => { const caId = e.target.value || null; edit((f) => ({ ...f, ca_id: caId })); if (caId) { const cc = await fetchCaCards(caId); edit((f) => ({ ...f, ...caInheritPatch(f, cc) })); } }}
              >
