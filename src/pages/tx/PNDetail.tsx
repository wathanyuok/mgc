...
import { fetchCaCards, caInheritPatch } from '@/lib/ca-inherit';
...
export function PNDetail({ mode }: { mode: 'new' | 'edit' }) {
  ...
  const tabs: TabDef[] = [
    ...
    },
    {
      key: 'accounting',
      label: 'Accounting',
      render: () => <AcctCards accounts={form.acct_cards} onChange={(n) => setForm((f) => ({ ...f, acct_cards: n }))} types={['CASH / BANK ACCOUNT', 'NOTE PAYABLE ACCOUNT', 'INTEREST EXPENSE ACCOUNT', 'ACCRUED INTEREST ACCOUNT']} />,
    },
    {
      key: 'chassis',
      label: 'Chassis',
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Select
              value={form.ca_id ?? ''}
              onChange={async (e) => { const caId = e.target.value || null; setForm((f) => ({ ...f, ca_id: caId })); if (caId) { const cc = await fetchCaCards(caId); setForm((f) => ({ ...f, ...caInheritPatch(f, cc) })); } }}
            >
